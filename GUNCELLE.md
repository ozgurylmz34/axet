...
| 10b | *(10'un İÇİNDE, otomatik — ayrı komut değil)* **CI ikamesi (sonra):** 6b'nin şartları sağlanıyorsa **ve** disk ağacı yayın etiketinin ağacıyla **birebir aynıysa** (yayın meta dosyaları hariç; plan beyanı değil, diskten ölçülür) adım 10 **test koşmaz** ve `[İKAME] sonra-ölçüm KOŞULMADI` + `KAPSAM` satırlarını basar. **AYNEN aktar.** Ağaç farklıysa `[ÖLÇ] … FARKLI (N yol: …)` basar ve normal ölçer. Yerel ortamın asgari kontrolü adım 12'de yine koşar. | 0 | — |
