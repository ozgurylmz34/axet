...
| BE-75 | Z mesaj sınıfı (`T100`) ya da Z domain (`DD07T`) yalnız `master_language`'de dolu → başka dilde login'de ham biçim (`tip:id:no v1 v2`) ya da çözücünün "bulamazsan kod döndür" dalı görünür; hata yok | Yeni/değişen Z mesaj/domain metni varsa hangi dillerin dolu olduğunu ölç (T100/DD07T dil kırılımı, `SELECT sprsl, COUNT(*) … GROUP BY sprsl`); tek dilliyse bilinçli karar olarak kaydet, çok dilliyse bulgu | WARNING | YOK | Ekip dersi (çeviri metni kullanıcı onaylı) |
| BE-76 | `WAIT UP TO n SECONDS`/`WAIT FOR` **örtük DB COMMIT** tetikler → RAP LUW'da illegal-statement dump'ı, ya da bekleyen commit'lenmemiş iş erken kalıcılaşır / `ENQUEUE … scope='2'`'de kilit erken düşer / açık imleç geçersizleşir. Statik hiçbir katman görmez (yalnız açık commit arayan tarama da kördür) | Deyimi eklemeden önce dört mekanizmayı ÖLÇ ve kodda yaz: RAP-LUW mu · bekleyen commit'lenmemiş iş var mı · kilit scope kaç · açık imleç var mı. Şerh **konuma** bağlıdır, deyimi başka yere taşımaz | BLOCKER | YOK | Ekip dersi; kardeşi BE-26 (açık commit) |
| BE-77 | Kod düzeltildi ama onu meşru gösteren yazılı sözleşme (CDS/BDEF şerhi, TS maddesi, `.rules.md`) aynı turda düzeltilmedi → kusur belgeye gömülü kalır, sonraki geliştirici belgeye uyup aynı kusuru yeniden doğurur | Her düzeltmede: bu kusuru hangi yazılı cümle meşru gösteriyordu · bu diff'te düzeltildi mi · düzeltme GÖRÜNÜR mü (eski cümle sessizce silinmez, "önceki X diyordu, yanlıştı, ölçüm Y" diye yazılır) | BLOCKER | YOK | Ekip dersi; tersi BE-18 (dokümante ama implemente değil) |
| BE-78 | Metot `EXPORTING et_msg TYPE bapiret2_t` ilan ediyor → çağrıda gövdeye girmeden aktüel tablo **ilk değerine döner** (boşalır); zincirlenmiş çağrıda (`me->a( IMPORTING et_msg = lt ). me->b( IMPORTING et_msg = lt ).`) ilk küme sessizce kaybolur | Zincirlenmiş çağrıda hedef tablo paylaşılıyor mu, imzada `EXPORTING` mi. Fix: her çağrıya ayrı tablo + `APPEND LINES OF` | WARNING | YOK | Ekip dersi; kuzeni BE-30 |
...
| BE-71 | `CALL FUNCTION … TABLES` aktüeli salt-okunur (çoğunlukla kendi `IMPORTING` param'ı doğrudan geçirilmiş) → çalışma zamanında `CX_SY_DYN_CALL_ILLEGAL_TYPE` ("write-protected"), dış sarmal `CX_SY_NO_HANDLER` gerçek sebebi kısaltabilir | `TABLES` bloğundaki her aktüeli çevreleyen metodun bildirimine götür: `IMPORTING` ise BULGU. Fix: `DATA(lt_x) = it_x.` yerel yazılabilir kopya, imza değişmez | BLOCKER | YOK | Ekip dersi; `sap-cds-ddic` `table-types.md` §5.4; kardeşi BE-69/BE-72 |
| BE-72 | `CALL FUNCTION … TABLES` aktüeli yerel `TYPES … WITH EMPTY KEY`; FM tabloyu içeride DDIC tablo tipli, referansla geçen bir `CHANGING`/`IMPORTING` parametresine devrediyor → uyumluluk aranır, `EMPTY KEY` ≠ DDIC standart anahtar (`DEFAULT KEY`) ⇒ çalışma zamanında `CX_SY_DYN_CALL_ILLEGAL_TYPE`; sözdizimi/aktivasyon/ATC geçer | Aktüelin bildirimine git: `EMPTY KEY` ise FM kaynağında devrin TİPLİ olup olmadığını oku (`PERFORM … TABLES`, `TABLES`→`TABLES` ve düz atama tipsizdir, sorun çıkarmaz). Fix: yerel tipi `WITH DEFAULT KEY` yap ya da DDIC tipini kullan; `EMPTY KEY`'e dayanan ölçütsüz `SORT`/`DELETE ADJACENT DUPLICATES`'i birlikte düzelt | BLOCKER | YOK | Ekip dersi; `sap-cds-ddic` `table-types.md` §5.2; kardeşi BE-71 |
| BE-73 | `COMMIT WORK AND WAIT`/`BAPI_TRANSACTION_COMMIT wait='X'` sonrası aynı akışta yaratılan belgeyi okuyan `SELECT`/`READ` **aralıklı** 0 satır dönebilir → guard "belge yok" der ya da sayaç yanlış "0 kalem" mesajı basar | Commit sonrası kendi yarattığın belgeyi okuyan erişim var mı; 0-satır dalı hangi kararı besliyor (guard/sayaç/mesaj). Fix: durum kapısı — ilk okuma daima, bulunursa hemen dön (sağlıklı yolda maliyet sıfır), bulunmazsa sınırlı tekrar, sonunda orijinal hata AYNEN | BLOCKER | YOK | Ekip dersi; komşusu BE-15/BE-26 |
| BE-74 | Üretici `BAPIRET2` satırını bilerek `message` alanı BOŞ kurar (id/number/v1-4 dolu, tüketicide çözülsün diye — meşru tasarım); tüketici çözücüyü çağırmazsa ekranda **bomboş satır**, hata/sy-subrc yok | `bapiret2` kuran atamalarda `message` bileşeni var mı; `-message` doğrudan okuyan her tüketici (RAP eşleyici, OData sonucu, ALV, posta, log) çözücüden geçiyor mu; çözücü TEK mi (ikinci kopya çözücü = sessiz ayrışma) | BLOCKER | YOK | Ekip dersi; komşusu BE-53 |
