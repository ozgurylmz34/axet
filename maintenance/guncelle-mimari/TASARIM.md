...
- **Kısayol (Z79):** SAP projesinde `KURULUMU-TAMAMLA.cmd` planın ayrı `KISAYOL` kalemidir (damga/Z55 deseni: tek başına da plan yazılır). Durum `yeni_proje.kisayol_durumu`: yok → yazılır · resmi işaretli ama `kisayol_metni()`'nden farklı → yedeklenip yeniden yazılır (`kisayol_yaz(guncelle=True)`) · işaretsiz → EZİLMEZ, bilgi satırı ("sil, tekrar çalıştır"). Onay kapsar, `uygula --otomatik` yazar, `geri-al` yedekten döner, `kapanis` ham bayt özetiyle doğrular. Metin tek kaynak `yeni_proje.py` (kopyalanmaz; yalnız SAP projesinde geç içe aktarılır). Yeni proje akışı (Z70) değişmedi: varsayılan kip ezmez.
