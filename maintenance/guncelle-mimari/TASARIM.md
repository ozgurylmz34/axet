...
- **gerektirir:** seçilen kalemin bağımlılığı seçilmemişse **ve önceki bir turda karşılanmamışsa** `sec` çıkış 2. **Z62 istisnası:** bağımlılık planda `yeniden_onerilen` (önceki turda ertelenmiş) bir kalemse `sec` DUR vermez, onu paketiyle birlikte **kendiliğinden seçer** ve stdout'a bir satırla söyler (`… önceki turda ertelenmişti ve yeniden önerildi — kendiliğinden seçildi`); zincir sabit noktaya kadar izlenir. Kapsam dar: `--cikar` ile açıkça dışlanan önkoşul ve yeniden önerilmemiş (normal bekleyen) önkoşul yine çıkış 2. *Karşılanmış* = `karsilanan` listesinde: yayını HEAD'in atası olan (içerilmiş) ya da `uygulanan.json`'da mühürlü kalem (plan bu kalemleri plana almaz; Z57, v0.5.3). Alan yoksa (eski motorun planı) karşılanmışlık ölçülemez ⇒ çıkış 2 (fail-closed). **`atlandi` da karşılanmış sayılır:** `uygulanan.json`'da `durum: atlandi` (hiçbir dosyası doğrulanmadı) olan kalem `karsilanan`'a girer, çünkü DUR bağımlıyı kalıcı kilitlerdi (atlandi kalem bir daha plana girmez). **Z58 (v0.5.4):** mühürün `neden` alanı (yukarıda `uygulanan.json`) `is-yok` ise kalem sessizce karşılanmıştır. Diğer her durum — `kabul`, tanınmayan değer ya da alan yok (`ertelendi` Z62'den beri buraya girmez: yeniden önerilir ya da dosyasızsa `is-yok` gibi karşılanır; `sec` Z58 motorunun planındaki `ertelendi` nedenini okumaya devam eder) (Z58 öncesi kayıt: ertelenmiş mi iş mi yoktu ayırt edilemez) — fail-closed olarak `karsilanan_atlandi`'ya da yazılır ve nedeni `karsilanan_atlandi_neden`'de taşınır (alan yoksa `null`). Bağ yalnız bu yolla karşılanıyorsa `sec` her bağ için bir `UYARI:` satırı basar (hangi kalem, hangi önkoşul, önkoşul `atlandi` + nedeni: "ertelenmiş" · "`kapanis --kabul` ile kapatılmış: dosyaları `bekliyor` kaldı" · "eski kayıt: ertelenmiş ya da iş yoktu, ayırt edilemiyor" · tanınmayan değer adıyla; dosyaları diskte olmayabilir) — çıkış kodu değişmez. `karsilanan_atlandi` yoksa (Z57 ilk sürümünün planı) boş kabul edilir; `karsilanan_atlandi_neden` yoksa (v0.5.3 planı) her kalemin nedeni bilinmiyor sayılır (eski-kayıt metni).
