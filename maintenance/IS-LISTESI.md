...
| Z16 | **`%guncelle` önce/sonra test turu: kapsam savunulamaz + `once` turunun çıkarımı geçersiz.** **ÖLÇÜLDÜ 2026-09-20** (canlı `%guncelle`, klon v0.1.0→v0.3.0, 17 kalem/40 dosya). ⓵ **Maliyet:** iki tam kök takımı turu ⇒ son kullanıcı için **1–1.5 saat**. Kullanıcı: *"son kullanıcılar bu şekilde sürekli 1-2 saat bekleyemez, sorun yaşarız"*. ⓶ **Kaynak:** `guncelle/harita.json`'da 39 sınıftan **3'ü filtresiz** `python tests/run_tests.py`'ye bağlı; ikisi bu planda var — `test-kok` (`glob: tests/*`, **12 dosya**, haritanın kendi beyanıyla `risk: dusuk`, `yukleme: "aXet çalışma zamanına yüklenmez"`) ve `bakim-script-kok` (`glob: scripts/doctor.py|session_brief.py|sap_stamp.py|behavior_manifest.py|project_precommit.py|merge_pr.py`, 3 dosya, `risk: yuksek`). ⇒ **en büyük maliyet, en zayıf gerekçede.** Diğer kök sınıfları zaten dar `-k` filtresi kullanıyor (`-k kur`, `-k install`, `-k new_` …). ⓷ **`once` turunun ÇIKARIMI GEÇERSİZ (yeni bulgu):** `komut_olc` testleri `calisma = (k.kok / cwd).resolve()` ile **klon kökünde** koşuyor ve `tests/*` güncellemenin parçası ⇒ `once` **eski** test kodunu, `sonra` **yeni** test kodunu koşuyor. `yeni_kirmizilar` (`guncelle.py:1317-1332`) ise komut kimliği bazında `cikis`/`failure` karşılaştırıyor. Testlerin kendisi değişirken *"fark = regresyon"* çıkarımı **kurulamaz** (bu turda `test_kural_hatirlatma.py` V2=yeni + v0.3.0'ın 9 yeni testi). ⓸ **Bu turda artık değer düşük:** plan `V3/V4 = 0` (39×V1 temiz alım, 3×V2, 433×V0) ⇒ uygulama sonrası ağaç **yayınlanan ağacın aynısı**; CI o ağacı **windows-latest + Py3.12/3.14** ile ölçtü (`.github/workflows/*.yml:37,153`), yani kullanıcının OS'i ve yorumlayıcısıyla aynı. ⛔ **Turu tümden kaldırmak YANLIŞ olur** — V3/V4>0 vakasında birleştirilmiş ağaç hiçbir yerde test edilmemiştir; ayrıca yerel ortam sapması (cp1254, MAX_PATH, antivirüs, eksik araç) ve yazma makinesi yalnız koşarak görülür. **Öneri (A+B):** **A** `test-kok` tam takım yerine değişen modülün dar süzgeci (`tests/_helpers.py` ya da `tests/run_tests.py` değişirse tam takım — paylaşılan altyapı). **B** yayına **`ci-durum.json`** eklensin (etiket · commit · takım · yeşil/kırmızı · OS/Python) ve `V3/V4 = 0` iken `olc --asama once` **koşmasın**, temel çizgi CI hükmü olsun; yalnız `sonra` koşsun. B hem ucuz hem **daha doğru** (CI yeni testleri yeni ürüne karşı temiz ortamda ölçtü). Kaba tahmin **60–80 dk → 10–20 dk**; kalan ağırlık `sap-adt-foundation` (yerelde ölçüldü: **409 test / 418 sn**). ⚠ `olc` için bugün **atlama seçeneği YOK** (grep: 0), zorunlu akış adımı. ⚠ Bu bir **gate açma** işi değil, var olanı daraltma — ADR 0019 moratoryumu kapsamı dışında. **KAPSAM — ölçülmeyen:** dar süzgecin modüller arası gizli bağımlılığı kaçırıp kaçırmadığı · `ci-durum.json`'ın güvenilirlik sınırı (yayıncı kendi hükmünü beyan ediyor) · V3/V4>0 vakasında gerçek süre. | tasarım turu (P-serisi) — son kullanıcı yayını ÖNCESİ |
| Z17 | **`CHANGELOG.md` ve `guncelle/yayinlar.json` tüketici klonuna hiç ulaşmıyor — üç yayında da.** `%guncelle plan` uyarısı: *"WARN beyansız EYLEM vakası: CHANGELOG.md (V1) kapsamda ama hiçbir kalemin `dosyalar` listesinde geçmiyor — UYGULANMAYACAK"* (aynısı `guncelle/yayinlar.json` için). **ÖLÇÜLDÜ 2026-09-20:** v0.1.0 · v0.2.0 · v0.3.0 → üçünde de bu iki dosya hiçbir kalemde beyan edilmiyor ⇒ **kronik ve yapısal**, tek bir yayının hatası değil. **Kök sebep:** `maintenance/yayin_hazirla.py` bu ikisini bilerek *"üretilen dosyalar"* sayıp kalem-diff kapsamından muaf tutuyor (aracın KAPSAM BEYANI'nda yazılı); tüketici motoru bu muafiyeti **bilmiyor**, kapsamda görüp beyansız bulunca atlıyor. İki taraf aynı kavramı iki ayrı yerde tanımlıyor. **Etkisi DÜŞÜK, işlev kırılmıyor:** motor `yayinlar.json`'ı zaten `origin/main`'den okuyor (`scripts/guncelle.py:361`), yerel kopyadan değil; `CHANGELOG.md` yalnız belge. Sonuç: tüketici klonunda bu iki dosya **bayat kalır** — kullanıcı `CHANGELOG.md`'ye bakarsa yanlış sürüm görür. **Seçenekler:** ⓐ yayın aracı her yayına bu iki dosyayı kapsayan otomatik bir *"yayın üstverisi"* kalemi eklesin ⓑ tüketici motoru üretilen-dosya listesini yayından okusun (tek kaynak) ⓒ dokunma, `CHANGELOG.md` klonda hiç tutulmasın. **KAPSAM — ölçülmeyen:** kullanıcının bayat `CHANGELOG.md`'yi fiilen okuyup okumadığı · başka üretilen dosya olup olmadığı. | Z16 ile aynı tasarım turu |
