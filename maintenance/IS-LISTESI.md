...
| Z46 | aXet bash `find` `-iname` desteklemiyor (arama hatası → yanlış "yok" riski) | motor içi bash emülasyonu mu, ölç; skill'lerde `grep -ri` / `rg -i` öner | 🟡 skill tarafı ✅ (`protocol.md`: `find` Go emülasyonu `-iname`/`-maxdepth` yok, 7 çağrıda ölçüldü → `rg --files --iglob`); **motor düzeltmesi açık** (motor bu repoda değil). Öneri: her oturum yüklenen `core/00-temel.md` "Kabuk ortamı" satırına da ekle (davranış yüzeyi → onay) |
