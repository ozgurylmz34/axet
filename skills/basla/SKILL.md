...
description: >
  ...
  kullanıcı %basla yazar. Kullanıcı "başla", "oturumu aç", "açılış özeti", "neredeyiz" derse de çalıştır.
---
...
1. **Özeti çalıştır:** proje `AGENTS.md` "Oturum" bölümündeki `session_brief.py` komutu. Bölüm yoksa bu
   skill'in bulunduğu template klonunun `scripts/session_brief.py`'si: `"<bu klasör>/../../scripts/session_brief.py"`.
...
2. **Yanıtı yaz — bu adım koşulsuzdur, atlanmaz.** Yanıtının ilk satırı çekirdek §0'daki kimlik satırıdır
